...
var Interaction = require('ol/interaction/Interaction');
 ...
 * @api
 */
class Grid extends Interaction__default["default"] {
  ...
  constructor(opt_options) {
     ...
     * @private
     */
    this.overlay_ = new VectorLayer__default["default"]({
     ...
     * @private
     */
    this.gridFeature_ = new Feature__default["default"]({
  ...
  updateGridDescription_() {
    ...
    // in 'EPSG:4326'
    const cp1 = proj.transform(this.originCoordinate, mapViewProj, 'EPSG:4326');

    const rotationAnchorCoordinate = this.rotationAnchorCoordinate || addVectors(this.originCoordinate, [1, 0]);

    const cp2 = proj.transform(rotationAnchorCoordinate, mapViewProj, 'EPSG:4326');

    const cp3 = [cp1[0], cp2[1]];
    const cp4 = [cp2[0], cp1[1]];

    const len = sphere.getDistance(cp1, cp2);
    let rise = sphere.getDistance(cp1, cp3);
    let run = sphere.getDistance(cp1, cp4);
  ...
  handleGridLayerPostRender_(event) {
    ...
    const localSphereNormalizationCoefficients = this
      .calculateLocalSphereNormalizationCoefficients(new geom.Point(gridDescription.originCoordinate));
    ...
    const viewExtentCenter = extent.getCenter(viewExtent);
    ...
    const vOrigin = new geom.Point(vOriginCoords);
  ...
  calculateLocalSphereNormalizationCoefficients(origin) {
    ...
    return [
      1 / sphere.getDistance(originCoords, testXCoords),
      1 / sphere.getDistance(originCoords, testYCoords),
    ];
  }
